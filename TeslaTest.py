...
dataFile.close()
ev3a = EV.EVidteslaEV_UpdateCloudInfo(ev2[0])
...
file.write(dataTemp )
file.close()
evCH2 = EV.teslaEV_GetChargingInfo(ev2[0])
evCH2a = EV.teslaEV_FastChargerPresent(ev2[0])
evCH2b = EV.teslaEV_ChargePortOpen(ev2[0])
evCH2c = EV.teslaEV_ChargePortLatched(ev2[0])
evCH2d = EV.teslaEV_GetBatteryLevel(ev2[0])
evCH2e = EV.teslaEV_MaxChargeCurrent(ev2[0])
evCH2f = EV.teslaEV_ChargeState(ev2[0])
evCH2h= EV.teslaEV_ChargingRequested(ev2[0])
evCH2i = EV.teslaEV_GetChargingPower(ev2[0])
evCH2j= EV.teslaEV_GetBatteryMaxCharge(ev2[0])

evCL2 = EV.teslaEV_GetClimateInfo(ev2[0])
evCL2a = EV.teslaEV_GetCabinTemp(ev2[0])
evCL2b = EV.teslaEV_GetOutdoorTemp(ev2[0])
evCL2c = EV.teslaEV_GetLeftTemp(ev2[0])
evCL2d = EV.teslaEV_GetRightTemp(ev2[0])
evCL2e = EV.teslaEV_GetSeatHeating(ev2[0])
evCL2f = EV.teslaEV_AutoConditioningRunning(ev2[0])
evCL2g = EV.teslaEV_PreConditioningEnabled(ev2[0])
evCL2h = EV.teslaEV_MaxCabinTempCtrl(ev2[0])
evCL2i = EV.teslaEV_MinCabinTempCtrl(ev2[0])
evCL2j = EV.teslaEV_SteeringWheelHeatOn(ev2[0])

evST = EV.teslaEV_GetStatusInfo(ev2[0])
evSTa = EV.teslaEV_GetCenterDisplay(ev2[0])
evSTb = EV.teslaEV_HomeLinkNearby(ev2[0])
evSTc = EV.teslaEV_GetLockState(ev2[0])
evSTd = EV.teslaEV_GetWindoStates(ev2[0])
evSTe = EV.teslaEV_GetOnlineState(ev2[0])
evSTf = EV.teslaEV_GetOdometer(ev2[0])
evSTg = EV.teslaEV_GetSunRoofState(ev2[0])
evSTh = EV.teslaEV_GetTrunkState(ev2[0])
evSTi = EV.teslaEV_GetFrunkState(ev2[0])
